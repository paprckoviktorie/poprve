    ...
    internal class Program
    {
        static int LinearSearch(int[] array, int elementToSearch)
        {
            ...
            for (int i = 0; i < array.Length - 1; i++)
            {
                ...
                {
                    Console.WriteLine($"Našel se prvek {i}");
                    return i;
                }
                else return -1;
            }
        ...
        static int BinarySearch(int[] array, int elementToSearch)
        {
                ...
                while (left <= right)
                {
                    ...
                    {
                        Console.WriteLine(" (Binární dělení dělilo interval " + internalDivisionCount + " krát.");
                        return mid; //střed je hledané číslo
                    }
            ...
            return -1; //když jsem nic nenašel, ten prvek tam nebyl
        }
        ...
        static int BinarySearchRecursive(int[] array, int elementToSearch, int lower, int upper)
        {
            ...
            //v každé proceduře si zvolím mid, ten vím, jak súočítat, když se náhodou úpotřebuju někam posunout, ZNOVU volám binární vyhledávání
            if (lower>upper)
                {
                    return -1; //nenalezeno
                }
                ...
                if ((array[mid] == elementToSearch)
                    {
                    return mid; //nalezeno v index mid
                    }
            if ((array[mid] < elementToSearch))
                {
